...
*A lightweight, language-agnostic fact parser, schema validator, and snippet adapter framework for Nabu.*
...
2. **Snippet Adapters:** Bi-directional snippet conversion and round-tripping for multiple structured formats including Markdown, JSON, and XML (`SnippetAdapter`, `snippetAdapterFactory`).
3. **Core Utility Library:** Standardized naming conventions (`NameMaker`, `QuantityNarrator`, `DocDate`), hierarchy traversal utilities (`iterUp`, `iterUps`, `find`, `findBehind`, `findNested`), and typed structured exception hierarchies (`Error`, `InternalError`, `ParsingError`, `FactFileError`).
...
To enable PDF generation support:

```bash
pip install protonabu[pdf]
```
